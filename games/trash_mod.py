...
class trash:
    ...
    def get_draw_deck_cards(self):
        visible_cards = self.player1['visibility'] + self.player2['visibility']
        all_cards = self.player1['deck'] + self.player2['deck'] + self.draw_deck + self.discard_deck
        cards_in_draw_deck = [card for card in all_cards if card not in visible_cards and card not in self.discard_deck]
        return cards_in_draw_deck
    
    def optimal_jack_placement(self, player):
        # Get the cards in the draw deck
        cards_in_draw_deck = self.get_draw_deck_cards()

        # Determine the cards the player needs
        needed_cards = [i+1 for i, val in enumerate(player['visibility']) if val == 'X']

        # Find the frequency of each needed card in the draw deck
        card_frequencies = {card: cards_in_draw_deck.count(card) for card in needed_cards}

        # Get the card with the least frequency (rarest in the draw deck)
        rarest_card = min(card_frequencies, key=card_frequencies.get)

        # Place the Jack in the position of the rarest card
        index = rarest_card - 1
        old_card = player['deck'][index]
        player['deck'][index] = 11  # Jack is represented by 11
        player['visibility'][index] = 11
        return old_card
    ...
    def play_turn(self, player, opponent):
        ...
        while self.can_place_card(card_in_hand, player):
            ...
            if card_in_hand == 11:
                card_in_hand = self.optimal_jack_placement(player)
            else:
